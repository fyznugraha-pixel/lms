...
export default function PekerjaanPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Pekerjaan & Produktivitas</h1>
        <p className="text-gray-500 mt-1">Kelola tugas pribadi Anda dan pantau aktivitas tim.</p>
      </div>
      ...
      <div className="flex space-x-1 bg-gray-100/50 p-1 rounded-xl w-fit border border-gray-200/50">
        <button
          ...
          className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${
            activeTab === 'SAYA' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'
          }`}
        >
          Pekerjaan Saya
        </button>
        <button
          ...
          className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${
            activeTab === 'FEED' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'
          }`}
        >
          Feed Perusahaan
        </button>
      ...
      {activeTab === 'SAYA' && (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[600px]">
          <div className="p-6 border-b border-gray-100 bg-gray-50/50">
            <h2 className="text-lg font-bold text-gray-900">To-Do List Pribadi</h2>
            <p className="text-sm text-gray-500 mt-1">Tugas yang perlu diselesaikan.</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {todos.length === 0 ? (
              <div className="text-center text-gray-400 py-10 text-sm">Belum ada tugas. Tambahkan sekarang!</div>
            ) : (
              todos.map(t => (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[600px]">
          <div className="p-6 border-b border-gray-100 bg-blue-50/50">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Work Log Harian</h2>
                <p className="text-sm text-gray-500 mt-1">Catat aktivitas kerja Anda hari ini.</p>
              </div>
          ...
          <form onSubmit={handleSaveWorkLog} className="flex-1 flex flex-col p-6 overflow-y-auto space-y-5">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Apa yang Anda selesaikan hari ini?</label>
              <textarea 
                required
                rows={4}
                value={dikerjakanHariIni}
                onChange={(e) => setDikerjakanHariIni(e.target.value)}
                placeholder="Misal: Menyelesaikan modul login, meeting dengan tim..."
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Apakah ada Blocker/Kendala? <span className="text-gray-400 font-normal">(Opsional)</span></label>
              <textarea 
                rows={2}
                value={blocker}
                onChange={(e) => setBlocker(e.target.value)}
                placeholder="Misal: Menunggu aset dari desainer..."
                className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm leading-relaxed"
              />
            </div>
