  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Rekapitulasi Karyawan</h1>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <h2 className="text-lg font-bold text-gray-900">Data Ringkasan Bulan Ini</h2>
        </div>
        
        {isLoading ? (
          <div className="p-12 text-center text-gray-500">Memuat data rekapitulasi...</div>
        ) : !data || data.ringkasan.length === 0 ? (
          <div className="p-12 text-center text-gray-500">Belum ada data kehadiran untuk bulan ini.</div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white text-gray-500 text-xs uppercase tracking-wider border-b border-gray-200">
                  ...
                  <th className="px-6 py-4 font-semibold text-center text-green-600">Hadir</th>
                  <th className="px-6 py-4 font-semibold text-center text-blue-600">Izin</th>
                  <th className="px-6 py-4 font-semibold text-center text-red-600">Sakit</th>
                  <th className="px-6 py-4 font-semibold text-center text-gray-900">Alpha</th>
                  <th className="px-6 py-4 font-semibold text-center text-orange-600">Incomplete</th>
                  <th className="px-6 py-4 font-semibold text-right text-indigo-600">Total Jam</th>
                </tr>
              </thead>
                ...
                {data.ringkasan.map((k: any) => (
                  <tr key={k.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right font-bold text-indigo-600">
                      {Math.floor(k.totalDurasiMenit / 60)}j {k.totalDurasiMenit % 60}m
                    </td>
                  </tr>
                ))}
              </tbody>
